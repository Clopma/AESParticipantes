(function() {
	...
	function ScheduleTemplate( element ) {
		...
		this.modalMaxWidth = 1500;
		this.modalMaxHeight = 1500;
			...
			httpRequest.onreadystatechange = function () {
				if (httpRequest.readyState === XMLHttpRequest.DONE) {
					if (httpRequest.status === 200) {
						self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML = self.getEventContent(httpRequest.responseText).outerHTML;
						Util.addClass(self.modal, 'cd-schedule-modal--content-loaded');
					}
				}
			};
			httpRequest.open('GET', window.location + content);
			httpRequest.send();
		}
	};
