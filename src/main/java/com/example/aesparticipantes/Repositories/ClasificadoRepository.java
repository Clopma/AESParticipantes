...
public interface ClasificadoRepository extends CrudRepository<Clasificado, Long> {
    ...
    @Query("select c.evento.categoria, c.participante, c.ronda from Clasificado c join Categoria ca where c.ronda like 'MEDALLA_%' and c.evento.competicion = ?1 order by ca.orden")
    List<Object[]> getMedallas(Competicion competicion);
